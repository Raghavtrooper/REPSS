...
def display_profile_details(profile_metadata: dict, use_inner_expanders: bool = True):
    ...
    with col1:
        st.write(f"**Title:** {profile_metadata.get('title', 'N/A')}")
        st.write(f"**Department:** {profile_metadata.get('department', 'N/A')}")
    with col2:
    ...
    def render_section(title, content):
        if not content:
            return
        display_content = ", ".join(content) if isinstance(content, list) else content
    ...
    render_section("Experience", profile_metadata.get('experience'))
    render_section("Education", profile_metadata.get('education'))
